 ...
 * @file lv_linux_drm.c
 *
 * MODIFIED from LVGL v9.5.0 upstream (MIT licensed, unchanged terms) for
 * devices/k230 - see devices/k230/README.md's UI section for the full
 * story. Upstream's DIRECT render mode has LVGL draw straight into the
 * raw DRM dumb buffer at the connector's *native* mode dimensions; that
 * buffer is allocated before lv_display_set_rotation() is ever called and
 * is never resized for it, so 90/270 rotation on this driver silently
 * clips content to the native (pre-rotation) width - confirmed on real
 * hardware (T-Display K230, 568x1232 native panel; rotated content was
 * compressed into the left ~46% of the logical width with the rest
 * blank). Every code path below for LV_DISPLAY_ROTATION_0 is untouched
 * from upstream; the new lv_linux_drm_set_rotation() function and the
 * added LV_DISPLAY_ROTATION_90/270 branches in lv_linux_drm_set_file()
 * and drm_flush() port the same software rotate-on-flush pattern
 * LVGL's own lv_linux_fbdev.c driver already uses for kernel framebuffer
 * drivers without hardware rotation support (lv_draw_sw_rotate() into a
 * correctly-native-strided scratch buffer, called from the flush
 * callback) - not a new algorithm, an existing first-party LVGL pattern
 * this driver never had ported to it.
 *
 * STATUS: SUPERSEDED, not compiled into the app - kept only for its
 * documented debugging history. A first bug here (lv_display_set_resolution()
 * called with hor/ver pre-swapped, which then got swapped *again* by
 * lv_display_set_rotation() itself, cancelling out) was found and fixed
 * below, but on-device output still showed a wrong pixel mapping after
 * that (not a clean 90-vs-270 mixup - two odd full-height streaks),
 * not root-caused. Before chasing that further, `nm -D` on
 * k230_phone_ui's own binary turned up the actual answer: Canaan/LILYGO's
 * own liblvgl.so (the *core* library, not liblvgl_linux.so) already has a
 * natively-built, working lv_linux_drm_set_rotation - that's what
 * k230_phone_ui itself calls. ui_app.cpp links against that instead now
 * (a plain `extern "C"` declaration, no header from this directory
 * needed) and rotation works correctly. Left in place as a documented
 * false start rather than deleted, in case the vendor symbol ever isn't
 * available in a future image and this needs picking back up - see
 * devices/k230/README.md's UI section.
 */
...
#endif
    /* Added: software rotate-on-flush support (see file header comment). */
    lv_display_rotation_t rotation;
    uint8_t * rotate_scratch_buf;
    size_t rotate_scratch_buf_size;
} drm_dev_t;
...
/* Added: must be called before lv_linux_drm_set_file() to select 90/270
 * software rotate-on-flush (see file header comment). ROTATION_0/180 are
 * unaffected - 180 was not exercised by this patch and keeps using
 * upstream's direct-mode path exactly as before. */
void lv_linux_drm_set_rotation(lv_display_t * disp, lv_display_rotation_t rotation)
{
...
lv_result_t lv_linux_drm_set_file(lv_display_t * disp, const char * file, int64_t connector_id)
{
    ...
    if(drm_dev->rotation == LV_DISPLAY_ROTATION_90 || drm_dev->rotation == LV_DISPLAY_ROTATION_270) {
        /* Added: software rotate-on-flush path (see file header comment).
         * LVGL renders into its own scratch buffer at the *rotated*
         * logical resolution; drm_flush() below transforms that into the
         * real native-stride dumb buffer before it reaches the hardware.
         * drm_dmabuf_set_active_buf's buffer-identity matching (used by
         * the direct-mode path below) has no meaning here since LVGL
         * isn't drawing into drm_bufs[] directly, so act_buf is fixed to
         * a single dumb buffer instead - one physical buffer, no
         * double-buffering, an acceptable tradeoff for a touch UI that
         * isn't rendering fast-moving video.
         */
        /* lv_display_set_rotation() swaps hor_res/ver_res itself (see its
         * own doc comment) - set the *native* resolution here (matching
         * the unmodified path below) and let that one call do the single
         * swap, rather than pre-swapping and having the two cancel out
         * (confirmed on real hardware: drm_flush() was seeing native
         * 568x1232 back, not the intended 1232x568, from exactly this
         * double-swap). */
        lv_display_set_resolution(disp, hor_res, ver_res);
        lv_display_set_rotation(disp, drm_dev->rotation);

        lv_color_format_t cf = lv_display_get_color_format(disp);
        uint32_t rotated_stride = lv_draw_buf_width_to_stride(ver_res, cf);
        size_t rotated_buf_size = (size_t) rotated_stride * (size_t) hor_res;
        drm_dev->rotate_scratch_buf = lv_malloc(rotated_buf_size);
        LV_ASSERT_MALLOC(drm_dev->rotate_scratch_buf);
        drm_dev->rotate_scratch_buf_size = rotated_buf_size;

        lv_display_set_buffers(disp, drm_dev->rotate_scratch_buf, NULL, rotated_buf_size,
...
static void drm_flush(lv_display_t * disp, const lv_area_t * area, uint8_t * px_map)
{
    ...
    if(drm_dev->rotation == LV_DISPLAY_ROTATION_90 || drm_dev->rotation == LV_DISPLAY_ROTATION_270) {
        /* Added: transform LVGL's rotated-logical-resolution render
         * (px_map, the scratch buffer set up in lv_linux_drm_set_file())
         * into the real native-orientation dumb buffer before it reaches
         * the hardware (see file header comment). */
        lv_color_format_t cf = lv_display_get_color_format(disp);
        ...
        lv_draw_sw_rotate(px_map, drm_dev->act_buf->map, src_w, src_h, src_stride,
                          (int32_t)drm_dev->act_buf->pitch, drm_dev->rotation, cf);
    }
    else {
        LV_UNUSED(px_map);
    }
