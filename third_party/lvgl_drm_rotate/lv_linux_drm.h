...
#ifndef LV_LINUX_DRM_ROTATE_H
#define LV_LINUX_DRM_ROTATE_H
...
/**
 * @brief Added (not upstream LVGL): select software rotate-on-flush for
 * this display - see lv_linux_drm.c's file header comment for why
 * upstream's lv_display_set_rotation() alone does not work with this
 * driver's DIRECT render mode. Call before lv_linux_drm_set_file().
 * LV_DISPLAY_ROTATION_0 (the default if never called) and
 * LV_DISPLAY_ROTATION_180 are unaffected and use the unmodified upstream
 * path; only 90/270 take the added code path.
 *
 * @param disp      Pointer to the display object created with lv_linux_drm_create()
